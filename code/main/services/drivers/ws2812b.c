...
void set_pixel(int index, unsigned char r, unsigned char g, unsigned char b, int write) {
  int bit;
...
void set_strip_color(unsigned char r, unsigned char g, unsigned char b) {
  int index;
  for (index = 0; index < REAL_LIGHT_COUNT; index += 1) {
    set_pixel(index, r, g, b, 0);
  }
  write_pixels();
}
